...
#ifndef _THRIFT_SERVER_TNONBLOCKINGSSLSERVER_H_
#define _THRIFT_SERVER_TNONBLOCKINGSSLSERVER_H_ 1
...
class TNonblockingSSLIOThread;

class TNonblockingSSLServer : public TServer {
private:
  class TConnection;

  friend class TNonblockingSSLIOThread;
...
private:
  ...
  static const int DEFAULT_IO_THREADS = 10;
  ...
  std::vector<boost::shared_ptr<TNonblockingSSLIOThread> > ioThreads_;
...
public:
  void setSSLSocketFactory(boost::shared_ptr<TSSLSocketFactory> factory)
  {
    factory_ = factory;
  }

  TNonblockingSSLServer(const boost::shared_ptr<TProcessorFactory>& processorFactory,
                     int port,
  ...
  TNonblockingSSLServer(const boost::shared_ptr<TProcessor>& processor,
                     int port,
                     const boost::shared_ptr<TSSLSocketFactory>& sslSocketFactory)
    : TServer(processor) {
    init(port);
...
class TNonblockingSSLIOThread : public Runnable {
public:
  ...
  bool notify(TNonblockingSSLServer::TConnection* conn);
...
private:
   ...
   * @param v void* callback arg where we placed TNonblockingSSLServer's "this".
   */
  static void listenHandler(evutil_socket_t fd, short which, void* v) {
    ((TNonblockingSSLServer*)v)->handleEvent(fd, which);
  }
