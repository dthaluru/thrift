 ...
 * So the SSL Server socket must be part of it. 
 */
class TNonblockingSSLServer::TConnection {
private:
  /// Server IO Thread handling this connection
  TNonblockingSSLIOThread* ioThread_;
  ...
  boost::shared_ptr<TSSLSocket> tSocket_;
...
public:
  ...
  static void eventHandler(evutil_socket_t fd, short /* which */, void* v) {
    assert(fd == static_cast<evutil_socket_t>(((TConnection*)v)->getTSSLSocket()->getSocketFD()));
  ...
  /// return the TSSLSocket transport wrapping this network connection
  boost::shared_ptr<TSSLSocket> getTSSLSocket() const { return tSocket_; }
...
class TNonblockingSSLServer::TConnection::Task : public Runnable {
public:
    ...
    try {
      for (;;) {
        if (serverEventHandler_) {
          serverEventHandler_->processContext(connectionContext_, connection_->getTSSLSocket());
        }
      ...
      GlobalOutput.printf("TNonblockingSSLServer: client died: %s", ttx.what());
    } catch (const bad_alloc&) {
      GlobalOutput("TNonblockingSSLServer: caught bad_alloc exception.");
      exit(1);
    } catch (const std::exception& x) {
      GlobalOutput.printf("TNonblockingSSLServer: process() exception: %s: %s",
                          typeid(x).name(),
                          x.what());
    } catch (...) {
      GlobalOutput.printf("TNonblockingSSLServer: unknown exception while processing.");
    }
...
void TNonblockingSSLServer::TConnection::workSocket() {
  int got = 0, left = 0;
  uint32_t fetch = 0;
  ...
  T_DEBUG("TConnection::workSocket(), SOCKET_RECV_FRAMING%s","\n");
    ...
    if (readWant_ > server_->getMaxFrameSize()) {
      ...
      GlobalOutput.printf(
          "TNonblockingSSLServer: frame size too large "
          "(%" PRIu32 " > %" PRIu64
          ") from client %s. "
...
bool TNonblockingSSLServer::getHeaderTransport() {
 ...
 * to, or finished receiving the data that it needed to.
 */
void TNonblockingSSLServer::TConnection::transition() {
  ...
  case APP_READ_REQUEST:
    ...
    if (server_->isThreadPoolProcessing()) {
      ...
      try {
        if (serverEventHandler_) {
          serverEventHandler_->processContext(connectionContext_, getTSSLSocket());
        }
        // Invoke the processor
        processor_->process(inputProtocol_, outputProtocol_, connectionContext_);
      } catch (const TTransportException& ttx) {
        GlobalOutput.printf(
            "TNonblockingSSLServer transport error in "
            "process(): %s",
            ttx.what());
...
void TNonblockingSSLServer::TConnection::setFlags(short eventFlags) {
...
void TNonblockingSSLServer::TConnection::checkIdleBufferMemLimit(size_t readLimit, size_t writeLimit) {
...
TNonblockingSSLServer::~TNonblockingSSLServer() {
  ...
  // The TNonblockingSSLIOThread objects have shared_ptrs to the Thread
  // objects and the Thread objects have shared_ptrs to the TNonblockingSSLIOThread
  // objects (as runnable) so these objects will never deallocate without help.
  while (!ioThreads_.empty()) {
    boost::shared_ptr<TNonblockingSSLIOThread> iot = ioThreads_.back();
    ioThreads_.pop_back();
 ...
 */
TNonblockingSSLServer::TConnection* TNonblockingSSLServer::createConnection(THRIFT_SOCKET socket,
                                                                      const sockaddr* addr,
                                                                      socklen_t addrLen) {
  // Check the stack
  Guard g(connMutex_);
 ...
 */
void TNonblockingSSLServer::returnConnection(TConnection* connection) {
  Guard g(connMutex_);
 ...
 */
void TNonblockingSSLServer::handleEvent(THRIFT_SOCKET fd, short which) {
  (void)which;
 ...
 */
void TNonblockingSSLServer::createAndListenOnSocket() {
#ifdef _WIN32
  ...
  if (error) {
    throw TException("TNonblockingSSLServer::serve() getaddrinfo "
  ...
  if (s == -1) {
    freeaddrinfo(res0);
    throw TException("TNonblockingSSLServer::serve() socket() -1");
  }
  ...
  if (::bind(s, res->ai_addr, static_cast<int>(res->ai_addrlen)) == -1) {
    ...
    throw TTransportException(TTransportException::NOT_OPEN,
                              "TNonblockingSSLServer::serve() bind",
                              THRIFT_GET_SOCKET_ERROR);
  }
 ...
 */
void TNonblockingSSLServer::listenSocket(THRIFT_SOCKET s) {
  // Set socket to nonblocking mode
  int flags;
  if ((flags = THRIFT_FCNTL(s, THRIFT_F_GETFL, 0)) < 0
      || THRIFT_FCNTL(s, THRIFT_F_SETFL, flags | THRIFT_O_NONBLOCK) < 0) {
    ::THRIFT_CLOSESOCKET(s);
    throw TException("TNonblockingSSLServer::serve() THRIFT_O_NONBLOCK");
  }
...
// See http://lists.danga.com/pipermail/memcached/2005-March/001240.html
  ...
  if (listen(s, LISTEN_BACKLOG) == -1) {
    ::THRIFT_CLOSESOCKET(s);
    throw TTransportException(TTransportException::NOT_OPEN, "TNonblockingSSLServer::serve() listen");
  }
...
void TNonblockingSSLServer::setThreadManager(boost::shared_ptr<ThreadManager> threadManager) {
  threadManager_ = threadManager;
  if (threadManager) {
    threadManager->setExpireCallback(
        apache::thrift::stdcxx::bind(&TNonblockingSSLServer::expireClose,
                                     this,
...
bool TNonblockingSSLServer::serverOverloaded() {
  ...
  if (numActiveProcessors_ > maxActiveProcessors_ || activeConnections > maxConnections_) {
    if (!overloaded_) {
      GlobalOutput.printf("TNonblockingSSLServer: overload condition begun.");
      overloaded_ = true;
    }
  } else {
    if (overloaded_ && (numActiveProcessors_ <= overloadHysteresis_ * maxActiveProcessors_)
      ...
      GlobalOutput.printf(
          "TNonblockingSSLServer: overload ended; "
          "%u dropped (%llu total)",
          nConnectionsDropped_,
          nTotalConnectionsDropped_);
...
bool TNonblockingSSLServer::drainPendingTask() {
  if (threadManager_) {
...
void TNonblockingSSLServer::expireClose(boost::shared_ptr<Runnable> task) {
  TConnection* connection = static_cast<TConnection::Task*>(task.get())->getTConnection();
  assert(connection && connection->getServer() && connection->getState() == APP_WAIT_TASK);
  connection->forceClose();
}

void TNonblockingSSLServer::stop() {
  if (!port_) {
    listenPort_ = 0;
  }
...
void TNonblockingSSLServer::registerEvents(event_base* user_event_base) {
  ...
  for (uint32_t id = 0; id < numIOThreads_; ++id) {
    ...
    shared_ptr<TNonblockingSSLIOThread> thread(
        new TNonblockingSSLIOThread(this, id, listenFd, useHighPriorityIOThreads_));
  ...
  GlobalOutput.printf("TNonblockingSSLServer: Serving on port %d, %d io threads.",
                      listenPort_,
                      ioThreads_.size());
 ...
 */
void TNonblockingSSLServer::serve() {
...
TNonblockingSSLIOThread::TNonblockingSSLIOThread(TNonblockingSSLServer* server,
                                           int number,
...
TNonblockingSSLIOThread::~TNonblockingSSLIOThread() {
  ...
  for (int i = 0; i < 2; ++i) {
    if (notificationPipeFDs_[i] >= 0) {
      if (0 != ::THRIFT_CLOSESOCKET(notificationPipeFDs_[i])) {
        GlobalOutput.perror("TNonblockingSSLIOThread notificationPipe close(): ",
                            THRIFT_GET_SOCKET_ERROR);
      }
...
void TNonblockingSSLIOThread::createNotificationPipe() {
  if (evutil_socketpair(AF_LOCAL, SOCK_STREAM, 0, notificationPipeFDs_) == -1) {
    GlobalOutput.perror("TNonblockingSSLServer::createNotificationPipe ", EVUTIL_SOCKET_ERROR());
  ...
  if (evutil_make_socket_nonblocking(notificationPipeFDs_[0]) < 0
    ...
    throw TException("TNonblockingSSLServer::createNotificationPipe() THRIFT_O_NONBLOCK");
  }
  for (int i = 0; i < 2; ++i) {
    ...
    if (evutil_make_socket_closeonexec(notificationPipeFDs_[i]) < 0) {
      ...
      ::THRIFT_CLOSESOCKET(notificationPipeFDs_[1]);
      throw TException(
          "TNonblockingSSLServer::createNotificationPipe() "
          "FD_CLOEXEC");
    }
  }
}
 ...
 */
void TNonblockingSSLIOThread::registerEvents() {
  ...
  if (number_ == 0) {
    GlobalOutput.printf("TNonblockingSSLServer: using libevent %s method %s",
                        event_get_version(),
  ...
  if (listenSocket_ >= 0) {
    ...
    event_set(&serverEvent_,
              listenSocket_,
              EV_READ | EV_PERSIST,
              TNonblockingSSLIOThread::listenHandler,
              server_);
  ...
  event_set(&notificationEvent_,
            ...
            EV_READ | EV_PERSIST,
            TNonblockingSSLIOThread::notifyHandler,
            this);
...
bool TNonblockingSSLIOThread::notify(TNonblockingSSLServer::TConnection* conn) {
...
/* static */
void TNonblockingSSLIOThread::notifyHandler(evutil_socket_t fd, short which, void* v) {
  TNonblockingSSLIOThread* ioThread = (TNonblockingSSLIOThread*)v;
  assert(ioThread);
  (void)which;

  while (true) {
    TNonblockingSSLServer::TConnection* connection = 0;
...
void TNonblockingSSLIOThread::breakLoop(bool error) {
  if (error) {
    GlobalOutput.printf("TNonblockingSSLServer: IO thread #%d exiting with error.", number_);
    // TODO: figure out something better to do here, but for now kill the
    // whole process.
    GlobalOutput.printf("TNonblockingSSLServer: aborting process.");
    ::abort();
  }
...
void TNonblockingSSLIOThread::setCurrentThreadHighPriority(bool value) {
#ifdef HAVE_SCHED_H
...
void TNonblockingSSLIOThread::stop() {
  // This should cause the thread to fall out of its event loop ASAP.
  breakLoop(false);
}

void TNonblockingSSLIOThread::join() {
